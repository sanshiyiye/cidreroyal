...
var addWare = function(b, g, f, c, j, d) {
  if (isNotBlank(f) && f) {
    ...
    var l = ajax(h, function() {
      window.location.href = "/order/order.action" + (isNotBlank(d) && (d != "") ? "?sid=" + $("#sid").val() : "")
    })
  } else {
    var e = createSpinner();
    $("#spinner2").show();
...
(function(f) {
  ...
  function g() {
    ...
    setTimeout(function() {
      ...
      document.getElementById(a.mainLayoutId).setAttribute("style", (m && m.indexOf("UCBROWSER") > -1) ? j + ";width:100%" : j);
      f.scrollTo(0, 0)
